...
const TrackIcon = ({ name, color }: { name: string, color: string }) => {
    ...
    let icon = <Music size={14} style={{ color }} />;
    if (n.includes('drum') || n.includes('beat')) icon = <Drum size={14} style={{ color }} />;
    else if (n.includes('bass') || n.includes('guitar')) icon = <Guitar size={14} style={{ color }} />;
    else if (n.includes('synth') || n.includes('piano') || n.includes('key')) icon = <Keyboard size={14} style={{ color }} />;
    else if (n.includes('voc') || n.includes('mic')) icon = <Mic size={14} style={{ color }} />;
    return icon;
};
...
const Arranger: React.FC<ArrangerProps> = ({ 
  ...
  // Auto-scroll
  useEffect(() => {
    if ((isPlaying || isRecording) && scrollContainerRef.current && autoScroll) {
        ...
        if (playheadX > visibleEnd * threshold) {
            container.scrollLeft = playheadX - (container.clientWidth * 0.1);
        }
    }
  }, [currentTime, isPlaying, isRecording, zoom, autoScroll]);
  ...
  const handleGlobalPointerDown = (e: React.PointerEvent) => {
      ...
      if (activePointers.current.size === 2) {
          ...
          // If tapping outside any specific interactive element, maybe start selection
  ...
  const handleGlobalPointerMove = (e: React.PointerEvent) => {
      ...
      if (activePointers.current.size === 2 && initialPinchDist.current) {
          ...
          return; // Skip other interactions if pinching
      }
    ...
    >
      ...
      <div className="flex-1 flex overflow-hidden relative">
             ...
             <div className="flex-1 overflow-hidden relative">
                 <div style={{ transform: `translateY(-${scrollContainerRef.current?.scrollTop || 0}px)` }}>
                    {project.tracks.map((track, idx) => (
                        <div 
                            key={track.id} 
                            className={`border-b border-zinc-800 relative group transition-colors select-none flex flex-col justify-center ${selectedTrackId === track.id ? 'bg-zinc-800' : 'bg-transparent'} ${trackDrag?.id === track.id ? 'opacity-50 ring-2 ring-studio-accent' : ''}`}
                            ...
                            <div className="px-2 pl-3 flex flex-col h-full py-2 justify-between relative">
                                ...
                                <div className="flex items-center space-x-2">
                                    <div 
                                        ...
                                        onPointerDown={(e) => handleTrackDragStart(e, track.id, idx)}
                                    >
                                    ...
                                    <span className={`font-bold text-zinc-200 truncate cursor-pointer hover:text-studio-accent ${isCompactHeader ? 'text-[10px]' : 'text-xs'}`} onClick={() => {
                                        const newName = prompt("Rename Track", track.name);
                                        if(newName && onRenameTrack) onRenameTrack(track.id, newName);
                                    }}>
                                        {track.name}
                                    </span>
                                </div>
                                ...
                                <div className="flex space-x-1 items-center pl-4">
                                    <button 
                                        onPointerDown={(e) => handlePointerDownAction(e, () => {})} 
                                    ...
                                    <button 
                                        onPointerDown={(e) => handlePointerDownAction(e, () => updateTrack(track.id, { muted: !track.muted }))}
                                    ...
                                    >M</button>
                                    <button 
                                        onPointerDown={(e) => handlePointerDownAction(e, () => updateTrack(track.id, { solo: !track.solo }))}
                                    ...
                                    <button
                                        onPointerDown={(e) => handlePointerDownAction(e, () => onOpenInspector(track.id))}
                                        className="w-4 h-4 rounded text-[8px] font-bold border border-zinc-700 flex items-center justify-center hover:bg-zinc-700 text-zinc-500 transition-colors sm:hidden"
                                        title="Settings"
                                    >
                                        <Settings size={8} />
                                    </button>
        ...
        <div 
            ...
            onPointerDown={(e) => {
                 // Selection start logic moved to global handle to avoid conflict with pinch
                 if (!dragState && !loopDrag && !trackDrag && !isScrubbing.active && activePointers.current.size === 1) {
                     // Still capture here if needed for scrolling but primarily handled globally or via specific elements
                 }
            }}
            onScroll={() => {
                // Sync Header
        ...
        >
            <div style={{ width: totalWidth, minWidth: '100%', height: project.tracks.length * trackHeight + 32 }}>
                ...
                <div 
                    ...
                    onDoubleClick={(e) => {
                        const time = calculateSeekTime(e.clientX, true); // Snap enabled? Default to true for markers usually
                ...
                <div className="relative" style={{ height: project.tracks.length * trackHeight }}>
                    ...
                    {/* Playhead */}
                    <div className="absolute top-0 bottom-0 w-px bg-red-500 z-50 pointer-events-none shadow-[0_0_4px_#ef4444]" style={{ left: currentTime * zoom }}>
