...
import { audioBufferToWav, makeDistortionCurve, getAutomationValueAtTime } from './utils';
...
interface AudioGraphChain {
    ...
    preFaderTap: GainNode; // Tap before gain/pan
    gain: GainNode; // Fader
    panner: StereoPannerNode;
    analyser?: AnalyserNode; 
    
    // Dual send nodes for Pre/Post switching without graph reconstruction
    reverbSendPre: GainNode;
    reverbSendPost: GainNode;
    delaySendPre: GainNode;
    delaySendPost: GainNode;
    chorusSendPre: GainNode;
    chorusSendPost: GainNode;
}
...
interface LiveTrackChannel extends AudioGraphChain {
    ...
    // Active Map for note tracking
    activeVoices: Map<number, SynthVoice>; 
    // Pool for reusing voice objects
    voicePool: SynthVoice[];
}
...
class AudioEngine {
  ...
  activeClipEnvelopes: Map<string, GainNode> = new Map();
  ...
  private _startTime: number = 0; // The AudioContext time when project time was 0
  private _pauseTime: number = 0; // The Project Time when paused
  ...
  // Optimization: Reusable buffer for RMS calculation
  ...
  constructor() {
    ...
    // @ts-ignore
    this.ctx.oninterruptionbegin = () => {
        this.stop();
        console.log("Audio interrupted");
    };
    // @ts-ignore
    this.ctx.oninterruptionend = () => {
        // Typically UI handles resume via overlay
    };
    ...
    this.rmsBuffer = new Float32Array(this.masterAnalyser.fftSize);
  ...
  initPeakWorker() {
      if (typeof Worker !== 'undefined') {
          try {
              this.peakWorker = new Worker(new URL('./peak.worker.ts', import.meta.url), { type: 'module' });
          } catch(e) {
              console.warn("Peak worker init failed, using main thread");
          }
      }
  }
  ...
  createTrackGraph(context: BaseAudioContext, destination: AudioNode): AudioGraphChain {
      ...
      const preFaderTap = context.createGain(); // Tap before Fader/Pan
      
      const gain = context.createGain(); // Fader
      const panner = context.createStereoPanner();
      
      // Dual Sends for Pre/Post capability
      const reverbSendPre = context.createGain();
      const reverbSendPost = context.createGain();
      const delaySendPre = context.createGain();
      const delaySendPost = context.createGain();
      const chorusSendPre = context.createGain();
      const chorusSendPost = context.createGain();
      ...
      compressor.connect(preFaderTap); // Tap Point
      preFaderTap.connect(gain);
      gain.connect(panner);
      panner.connect(destination);

      // Connect Pre Sends (Source: preFaderTap)
      preFaderTap.connect(reverbSendPre);
      preFaderTap.connect(delaySendPre);
      preFaderTap.connect(chorusSendPre);

      // Connect Post Sends (Source: panner)
      panner.connect(reverbSendPost);
      panner.connect(delaySendPost);
      panner.connect(chorusSendPost);

      return {
          input, distortionNode, lowFilter, midFilter, highFilter, compressor,
          preFaderTap, gain, panner, 
          reverbSendPre, reverbSendPost,
          delaySendPre, delaySendPost,
          chorusSendPre, chorusSendPost
      };
  }
  ...
  getTrackChannel(trackId: string): LiveTrackChannel {
    if (!this.trackChannels.has(trackId)) {
      ...
      chain.gain.disconnect(); // We reconnect specifically for metering or other needs if required, but graph is connected in createTrackGraph
      
      // Re-establish connection to destination (since we disconnected gain, we broke chain? No, gain->panner->destination)
      // Wait, createTrackGraph connects panner to destination.
      // We only disconnect gain if we want to insert metering or analysis.
      
      const analyser = this.ctx.createAnalyser();
      analyser.fftSize = 256;
      analyser.smoothingTimeConstant = 0.5;
      
      // Re-route gain -> analyser -> panner
      chain.gain.disconnect();
      chain.gain.connect(analyser);
      analyser.connect(chain.panner);

      // Connect Sends to Main Bus
      chain.reverbSendPre.connect(this.reverbInput);
      chain.reverbSendPost.connect(this.reverbInput);
      chain.delaySendPre.connect(this.delayInput);
      chain.delaySendPost.connect(this.delayInput);
      chain.chorusSendPre.connect(this.chorusInput);
      chain.chorusSendPost.connect(this.chorusInput);
  ...
  // Voice Management
  private acquireVoice(channel: LiveTrackChannel, config: InstrumentConfig): SynthVoice {
      let voice = channel.voicePool.find(v => !v.isBusy);
      if (!voice) {
          voice = new SynthVoice(this.ctx, channel.input, config);
          channel.voicePool.push(voice);
      }
      return voice;
  }
  ...
  triggerNoteAttack(trackId: string, note: number, config: InstrumentConfig, velocity: number = 127) {
      ...
      const voice = this.acquireVoice(channel, config);
  ...
  play(clips: Clip[], tracks: Track[], startTime: number = 0) {
    ...
    this.syncTracks(tracks, startTime);
    
    // Schedule audio clips immediately (Web Audio scheduler handles precision)
    clips.forEach(clip => {
  ...
  stop() {
    ...
    this.activeClipEnvelopes.clear();
    ...
    this.trackChannels.forEach(ch => {
        ...
        ch.panner.pan.cancelScheduledValues(this.ctx.currentTime);
        ch.reverbSendPre.gain.cancelScheduledValues(this.ctx.currentTime);
        ch.reverbSendPost.gain.cancelScheduledValues(this.ctx.currentTime);
        ch.delaySendPre.gain.cancelScheduledValues(this.ctx.currentTime);
        ch.delaySendPost.gain.cancelScheduledValues(this.ctx.currentTime);
        ch.chorusSendPre.gain.cancelScheduledValues(this.ctx.currentTime);
        ch.chorusSendPost.gain.cancelScheduledValues(this.ctx.currentTime);
  ...
  // Convert an absolute AudioContext time to Project Time
  ...
  // Recorder Delegation
  async startRecording(monitoring: boolean): Promise<number> {
  ...
  async initInput(deviceId?: string) { return this.recorder.initInput(deviceId); }
  closeInput() { this.recorder.closeInput(); }
  
  get selectedInputDeviceId() { return this.recorder.selectedInputDeviceId; }
  set selectedInputDeviceId(id: string | undefined) { this.recorder.selectedInputDeviceId = id; }

  // Other Public Methods
  async resumeContext() { if (this.ctx && this.ctx.state === 'suspended') await this.ctx.resume(); }
  public clearBuffers() {
    this.stop();
    this.buffers.clear();
    this.peaks.clear();
  ...
  private disconnectChannel(channel: AudioGraphChain) {
      try {
          ...
          channel.preFaderTap.disconnect();
          channel.reverbSendPre.disconnect();
          channel.reverbSendPost.disconnect();
          channel.delaySendPre.disconnect();
          channel.delaySendPost.disconnect();
          channel.chorusSendPre.disconnect();
          channel.chorusSendPost.disconnect();
          if ((channel as LiveTrackChannel).voicePool) {
              (channel as LiveTrackChannel).voicePool.forEach(v => v.disconnect());
          }
      } catch (e) {}
  }
  ...
  async setOutputDevice(deviceId: string) {
      if (!this.ctx) return;
      // @ts-ignore
      if (typeof this.ctx.setSinkId === 'function') await this.ctx.setSinkId(deviceId);
  }

  setClipGain(clipId: string, gain: number) {
      const envelope = this.activeClipEnvelopes.get(clipId);
      if (envelope) envelope.gain.setTargetAtTime(gain, this.ctx.currentTime, 0.02);
  }

  private getRMS(analyser: AnalyserNode) {
    if (!this.rmsBuffer) return 0;
    analyser.getFloatTimeDomainData(this.rmsBuffer);
    let sum = 0;
    for (let i = 0; i < this.rmsBuffer.length; i++) sum += this.rmsBuffer[i] * this.rmsBuffer[i];
  ...
  // ... [Scheduler, ApplyTrackSettings, RenderProject, Metronome logic same as before but using SynthVoice class]
  
  applyTrackSettings(chain: AudioGraphChain, track: Track, currentTime: number, isOffline: boolean, seekTime?: number) {
      const { 
          gain, panner, lowFilter, midFilter, highFilter, compressor, distortionNode, 
          reverbSendPre, reverbSendPost, 
          delaySendPre, delaySendPost, 
          chorusSendPre, chorusSendPost 
      } = chain;
      ...
      // Volume Logic: Automation overrides static volume if present
      let targetVolume = isMuted ? 0 : track.volume;
      if (track.automation?.volume && track.automation.volume.length > 0 && seekTime !== undefined && !isMuted) {
          targetVolume = getAutomationValueAtTime(track.automation.volume, seekTime, track.volume);
      }
      
      if (isOffline || prev.volume !== targetVolume || prev.muted !== isMuted || seekTime !== undefined) {
          if (!isOffline) gain.gain.cancelScheduledValues(currentTime);
          if (isOffline || seekTime !== undefined) gain.gain.setValueAtTime(targetVolume, currentTime);
      ...
      if (comp && comp.enabled) {
          if (isOffline || prevComp?.threshold !== comp.threshold) {
              isOffline ? (compressor.threshold.value = comp.threshold) : compressor.threshold.setTargetAtTime(comp.threshold, currentTime, 0.1);
          }
          if (isOffline || prevComp?.ratio !== comp.ratio) {
              isOffline ? (compressor.ratio.value = comp.ratio) : compressor.ratio.setTargetAtTime(comp.ratio, currentTime, 0.1);
          }
      }
      
      // Sends Logic with Pre/Post switching
      const config = track.sendConfig || { reverbPre: false, delayPre: false, chorusPre: false };
      
      const updateSend = (
          preNode: GainNode, postNode: GainNode, 
          value: number, isPre: boolean, prevVal?: number, prevPre?: boolean
      ) => {
          if (isOffline || prevVal !== value || prevPre !== isPre) {
              const targetPre = isPre ? value : 0;
              const targetPost = isPre ? 0 : value;
              
              if (isOffline) {
                  preNode.gain.value = targetPre;
                  postNode.gain.value = targetPost;
              } else {
                  preNode.gain.setTargetAtTime(targetPre, currentTime, 0.05);
                  postNode.gain.setTargetAtTime(targetPost, currentTime, 0.05);
              }
          }
      };

      updateSend(reverbSendPre, reverbSendPost, track.sends.reverb, config.reverbPre, prev.sends?.reverb, prev.sendConfig?.reverbPre);
      updateSend(delaySendPre, delaySendPost, track.sends.delay, config.delayPre, prev.sends?.delay, prev.sendConfig?.delayPre);
      updateSend(chorusSendPre, chorusSendPost, track.sends.chorus, config.chorusPre, prev.sends?.chorus, prev.sendConfig?.chorusPre);
  ...
  syncTracks(tracks: Track[], seekTime?: number) {
    if (!this.ctx) return;
    ...
    tracks.forEach(track => {
      ...
      this.applyTrackSettings(channel, effectiveTrack, currentTime, false, seekTime);
    });
  }
  ...
  scheduleSource(buffer: AudioBuffer, when: number, offset: number, playDuration: number, destination: AudioNode, clip: Clip, elapsedClipTime: number, ctx: BaseAudioContext = this.ctx) {
    ...
    if (ctx === this.ctx) {
        ...
        this.activeClipEnvelopes.set(clip.id, envelope);
        source.onended = () => {
            if (this.activeSources.has(sourceId)) this.activeSources.delete(sourceId);
            if (this.activeClipEnvelopes.has(clip.id)) this.activeClipEnvelopes.delete(clip.id);
        };
    }
  }

  // [Previous methods like setMasterVolume, setMasterEq etc. remain same but simplified here for brevity]
  ...
  async playCountIn(bars: number, bpm: number) {
      ...
      return new Promise<void>(resolve => setTimeout(resolve, beats * interval * 1000));
  }
  ...
  // Tanpura & Tabla Helpers
  getTanpuraFreqs(config: TanpuraState): number[] {
      const rootFreq = NOTE_FREQS[config.key] || 261.63;
      const sa = rootFreq;
      const saLow = sa / 2;
      let firstStringFreq = saLow * 1.5;
  ...
  scheduler(projectTracks?: Track[], projectClips?: Clip[]) {
    ...
    if (projectTracks) {
        const now = this.ctx.currentTime;
        const schedulerWindowStart = now - this._startTime;
        const schedulerWindowEnd = schedulerWindowStart + lookahead;

        projectTracks.forEach(track => {
            if (track.automation?.volume && track.automation.volume.length > 0) {
                const channel = this.trackChannels.get(track.id);
                if (channel) {
                    track.automation.volume.forEach(point => {
                        if (point.time >= schedulerWindowStart && point.time < schedulerWindowEnd) {
                            const schedTime = this._startTime + point.time;
                            if (point.curve === 'step') {
                                channel.gain.gain.setValueAtTime(point.value, schedTime);
                            } else if (point.curve === 'exponential') {
                                // Exp ramp needs non-zero start/end. Clamp to 0.001 (-60dB)
                                const val = Math.max(0.001, point.value);
                                channel.gain.gain.exponentialRampToValueAtTime(val, schedTime);
                            } else {
                                channel.gain.gain.linearRampToValueAtTime(point.value, schedTime);
                            }
                        }
                    });
                }
            ...
            projectClips.forEach(clip => {
                ...
                clip.notes.forEach(note => {
                    ...
                    while (iterationsChecked < 100) { 
                        const absStart = baseTime + (kLoop * loopLength);
                        if (absStart >= schedulerWindowEnd) break;
                        ...
                        if (absStart >= schedulerWindowStart) {
                             ...
                             this.scheduleNote(track.id, note.note, schedTime, duration, track.instrument!, note.velocity);
                        }
  ...
  async renderProject(project: ProjectState): Promise<Blob> {
      const maxTime = Math.max(project.loopEnd, ...project.clips.map(c => c.start + c.duration)) + 2; 
      ...
      for (const track of project.tracks) {
          ...
          for (const clip of trackClips) {
              ...
              if (clip.notes && track.type === 'instrument' && track.instrument) {
                  // ... [MIDI Render Logic same as scheduler loop but for offline] ...
                  const loopLength = clip.loopLength || clip.duration;
                  clip.notes.forEach(note => {
                      const baseTime = clip.start - (clip.offset||0) + note.start;
                      let k = 0;
                      let iterationsChecked = 0;
                      while(iterationsChecked < 100) {
                          const absStart = baseTime + (k * loopLength);
                          if (absStart >= clip.start + clip.duration) break;
                          if (absStart >= clip.start) {
                              const freq = midiToFreq(note.note);
                              const voice = new SynthVoice(offlineCtx, trackChain.input, track.instrument!);
                              voice.play(freq, note.velocity, absStart, track.instrument!);
                              voice.triggerRelease(absStart + note.duration);
                          }
                          if (loopLength <= 0) break;
                          k++;
                          iterationsChecked++;
