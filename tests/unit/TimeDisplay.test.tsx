...
describe('TimeDisplay', () => {
  it('formats bars correctly', () => {
    ...
    // 0s -> 1:1:1
    const { getByText, rerender } = render(<TimeDisplay currentTime={0} bpm={120} />);
    expect(getByText('1:1:1')).toBeInTheDocument();

    // 0.5s (1 beat) -> 1:2:1
    rerender(<TimeDisplay currentTime={0.5} bpm={120} />);
    expect(getByText('1:2:1')).toBeInTheDocument();
    
    // 2.0s (1 bar) -> 2:1:1
    rerender(<TimeDisplay currentTime={2.0} bpm={120} />);
    expect(getByText('2:1:1')).toBeInTheDocument();
  });

  it('toggles mode on click', () => {
    const { getByText } = render(<TimeDisplay currentTime={65} bpm={120} />);
